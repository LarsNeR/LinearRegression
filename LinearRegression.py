...
class LinearRegression(object):
    ...
    def fit(self, X, y):
        ...
        self.weights_tmp = np.zeros(1 + X.shape[1])
    ...
    def gradient_descent(self, X, y):
        ...
        self.weights_tmp[0] = self.weights[0] - self.eta / len(y) * self.sigma(X, y, np.ones(X.shape[0]))

        # Update all weights with gradient descent (in weights_tmp)
        for j in range(len(self.weights) - 1):
            self.weights_tmp[j + 1] = self.weights[j + 1] - self.eta / len(y) * self.sigma(X, y, X[:, j])

        # Update all weights simultaneously
        self.weights = self.weights_tmp
